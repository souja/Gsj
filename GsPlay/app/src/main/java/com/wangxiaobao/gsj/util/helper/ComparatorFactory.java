...
public class ComparatorFactory {
    ...
    private static int compareIndexValue(String lhs, String rhs) {
        ...
        if (ConvertHelper.INDEX_SPECIAL.equals(lhs)) {
            //索引值排序时#排在前面
            return ConvertHelper.INDEX_SPECIAL.equals(rhs) ? 0 : 1;
        } else if (ConvertHelper.INDEX_SPECIAL.equals(rhs)) {
            return -1;
        }
    ...
    private static int compareIndexEntity(IndexStickyEntity lhs, IndexStickyEntity rhs) {
        ...
        if (!ConvertHelper.INDEX_SPECIAL.equals(lhsIndexValue) && !ConvertHelper.INDEX_SPECIAL.equals(rhsIndexValue)) {
            //两者的索引值均不为：#，则比较时只需要按照pinyin比较自然大小即可
            return lhs.getPinYin().compareTo(rhs.getPinYin());
        } else if (!ConvertHelper.INDEX_SPECIAL.equals(lhsIndexValue) && !ConvertHelper.INDEX_SPECIAL.equals(rhsIndexValue)) {
            //两者索引值均为：#，则比较时按照pinyin比较自然大小即可
            return lhs.getPinYin().compareTo(rhs.getPinYin());
        } else {
            //两者中只有一个索引值为:#，则索引值为#的小
            return ConvertHelper.INDEX_SPECIAL.equals(lhsIndexValue) ? 1 : -1;
        }
    }
}
